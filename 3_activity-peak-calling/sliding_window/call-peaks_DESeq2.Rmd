...
This is an r markdown script that details the activity analysis and results of the ATAC-STARR dataset. Briefly, active regions are called using DESeq2 which performs differential analysis on the counts matrix that is fed in. The active must have a padj (Benjamin-Hochberg corrected p-value) < 0.1 and a Log2(RNA/DNA "foldchange") > 0. I also extract silent regions, padj < 0.1, L2FC < 0. I then isolate these regions, add positional information using the cts file, and write as a tsv for downstream analysis. Importantly, I shrink the lfc values using the apeglm estimator. 
